...
class mem_update(nn.Module):
    ...
    def __init__(self,c1,c2):
        ...
        # if c1==3 and c2==16:
        # self.leakec = nn.Parameter(torch.ones(timestep, c2,3,3) * 0.5)
        # self.inputc = nn.Parameter(torch.ones(timestep, c2,3,3) * 0.5)
        # self.resetc = nn.Parameter(torch.ones([timestep, c2,3,3]))
        self.leake = nn.Parameter(torch.ones(timestep,c2) * 0.5)
        self.input = nn.Parameter(torch.ones(timestep,c2) * 0.5)
        self.reset = nn.Parameter(torch.ones([timestep,c2 ]) )
        # self.e = nn.Parameter(torch.zeros([timestep, c2]))
        # self.b = nn.Parameter(torch.ones([timestep, c2])* 0.5)
        # self.r = nn.Parameter(torch.zeros([timestep, c2])*0.2)
        # self.leake = nn.Parameter(torch.ones(timestep) * 0.5)
        # self.input = nn.Parameter(torch.ones(timestep) * 0.5)
        # self.reset = nn.Parameter(torch.ones([timestep]))
    ...
    def forward(self, x_seq):
        ...
        # mem = x_seq[-1]
        mem = 0
        # mem = torch.zeros(x_seq[DA0].shape)
        # device=x_seq.device
        # mem=mem.to(device)
        spike_seq = []
        # relu=nn.ReLU()
        for t in range(timestep):
            ...
            #注意
            mem =self.leake[t].view([1,x_seq.size(2),1,1])* mem + self.input[t].view([1,x_seq.size(2),1,1])* x_seq[t]
            mem_thr = mem/self.reset[t].view([1,x_seq.size(2),1,1])-0.5
            x = self.spike_fn(mem_thr)
            mem = mem - x*self.reset[t].view([1,x_seq.size(2),1,1])
            #

            #ulif#
            # mem = self.leake[0].view([1, x_seq.size(2), 1, 1]) * mem + self.input[0].view([1, x_seq.size(2), 1, 1]) * x_seq[t]
            # mem_thr = mem  - 0.75
            # x = self.spike_fn(mem_thr)
            # mem = mem - x
            # plif#
            # leak=self.leake[1].view([1, x_seq.size(2), 1, 1])
            # mem = self.sigmoid(leak) * mem +  (1-self.sigmoid(leak))* x_seq[t]
            # mem_thr = mem  - 1
            # x = self.spike_fn(mem_thr)
            # mem = mem - x

            # lif#y
            # mem = 0.5 * mem + 0.5 * x_seq[t]
            # mem_thr = mem  - 1
            # x = self.spike_fn(mem_thr)
            # mem = mem - x
...
class TA(nn.Module):
    def __init__(self,  T,ratio=2):
        ...
        self.sharedMLP = nn.Sequential(
            nn.Conv3d(T, T // ratio, 5,padding=2 ),
            nn.SiLU(),
            nn.Conv3d(T // ratio, T, 5,padding=2),
        )
        self.sigmoid = nn.Sigmoid()
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.gac=False
        if c1==3 and c2==16:
            self.encoding = GAC(T=timestep, out_channels=16)
            self.gac=True
            self.conv = nn.Conv3d(c1, c2, (3, k, k), (1, s, s), (1, autopad(k, p, d), autopad(k, p, d)), groups=g,
                                  dilation=d,
                                  bias=False)
            self.bn = nn.BatchNorm3d(c2)
        else:
            self.conv2D = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
            self.bn2D = nn.BatchNorm2d(c2)
            # self.conv = nn.Conv3d(c1, c2, (3, k, k), (1, s, s), (1, autopad(k, p, d), autopad(k, p, d)), groups=g,
            #                       dilation=d,
            #                       bias=False)
            # self.bn = nn.BatchNorm3d(c2)
        # self.conv = nn.Conv3d(c1, c2, (3, k, k), (1, s,  s), (1+1, autopad(k, p, d), autopad(k, p, d)), groups=g, dilation=(2,d,d),
        #                       bias=False)

        # self.conv = nn.Conv3d(c1, c2, (1, 1, 1), (1, s, s), (3, autopad(k, p, d), autopad(k, p, d)), groups=g,
        #                       dilation=d,
        #                       bias=False)
        # self.bn = nn.BatchNorm3d(c2)
        # self.conv = nn.Conv3d(c1, c2, (1, k, k), (1, s, s), (0, autopad(k, p, d), autopad(k, p, d)), groups=g,
        #                       dilation=d,
        #                       bias=False)
        # self.bn = nn.BatchNorm3d(c2)
        # self.conv = nn.Conv3d(timestep, timestep, (1, 1, 1), (1, 1, 1), (0, 0, 0), groups=g,
        #                       dilation=d,
        #                       bias=False)
        # self.bn = nn.BatchNorm3d(timestep)
        # self.conv2 = nn.Conv3d(timestep, timestep, (3, 3, 3), (1, 1, 1), (1, 1, 1), groups=timestep,
        #                       dilation=d,
        #                       bias=False)
        # self.bn2 = nn.BatchNorm3d(timestep)

        #     for _ in range(timestep):
        #         self.bn.append(nn.BatchNorm2d(c1) )
        #
        #     self.bn2 = nn.ModuleList()
        #     for _ in range(timestep):
        #         self.bn2.append(nn.BatchNorm2d(c2))
        # self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        self.act=nn.LeakyReLU()
        self.mem=mem_update(c1,c2)
        self.spike_fn = sigmoid.apply
        # self.mem2 = mem_update()
        # self.mem3 = mem_update()
    def forward(self, x):
        ...
        if self.gac:
            ...
            # (t,b,c,h,w)
            # (b,c,t,h,w)
            x = torch.reshape(x, [timestep, -1, x.size(1), x.size(2), x.size(3)]).permute(1, 2, 0, 3, 4)
            x = self.bn(self.conv(x)).permute(2, 0, 1, 3, 4)
            imgs = x
            x = self.mem(x)
            # x = self.spike_fn(x)

            x = self.encoding(imgs.transpose(0, 1), x.transpose(0, 1)).transpose(0, 1)
            ...
            x1=self.bn2D(self.conv2D(x))
            _, c, h, w = x1.shape
            x1 = torch.reshape(x1, [timestep, -1, c, h, w])
            x = self.mem(x1)
            #
            #
            # #
            return x.flatten(0,1)# 改maichong

        # if self.k==1 or x.size(1)==3:
        #     x=self.bn(self.conv(x))
        #     _, c, h, w = x.shape
        #     x = torch.reshape(x, [timestep, -1, c, h, w])
        #     x=self.mem(x)
        #
        #     return x.flatten(0,1)#改maichong
        # else:
        ...
        #         lis.append(self.bn[i](self.conv[i](x[i])))
        #     x=torch.stack(lis,dim=0)
        #     x=self.mem(x)
        #     lis = []
        #     for i in range(timestep):
        #         lis.append(self.bn2[i](self.conv2[i](x[i])))
        #     x = torch.stack(lis, dim=0)
        #     x = self.mem2(x)
        #     return x.flatten(0, 1)

    def forward_fuse(self, x):
        """Applies a fused convolution and a  ctivation function to the input tensor `x`."""
        if self.gac:
            # _, c, h, w = x.shape
            # (t,b,c,h,w)
            # (b,c,t,h,w)
...
class BottleneckCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        # x=self.act(self.bn(torch.cat((y1, y2), 1)))

        x=self.bn(torch.cat((y1, y2), 1))
        _, c, h, w = x.shape()

        x = self.mem(x.reshape(timestep, -1, c, h, w)).flatten(0,1)
        return self.cv4(x)
...
class Detections:
    ...
    def save(self, labels=True, save_dir="runs/detect/224 48 time6 direct yolov5s", exist_ok=False):
        """
        Saves detection results with optional labels to a specified directory.

        Usage: save(labels=True, save_dir='runs/detect/224 48 time6 direct yolov5s', exist_ok=False)
        """
        save_dir = increment_path(save_dir, exist_ok, mkdir=True)  # increment save_dir
        self._run(save=True, labels=labels, save_dir=save_dir)  # save results

    def crop(self, save=True, save_dir="runs/detect/224 48 time6 direct yolov5s", exist_ok=False):
        """
